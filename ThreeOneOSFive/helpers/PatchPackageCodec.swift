...
enum PatchPackageCodec {
    ...
    // Schema v3 and the supplied schema v9 packages use the same authenticated
    // envelope and AES-GCM payload format with versioned AAD. Keep the version
    // range centralized so inspect/decode can accept both generations.
    ...
    // XRE protects the newly supplied ATH packages. Keep the legacy password
    // as a fallback so existing xTop1 bundled packages continue to load.
    static let bundledResourcePasswords = [
        "XRE",
        "xTop1-External-Internal-Patch-Key-2026"
    ]
