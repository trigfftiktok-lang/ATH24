...
enum PatchProjectLibrary {
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        ...
        // Different UI buttons may intentionally ship packages with the same
        // internal project UUID but different bundle filenames (for example
        // FF NORMAL vs FF MAX). Keep each filename as a distinct library item
        // so button mappings never fail with PACKAGE NOT FOUND.
        ...
        for url in urls where url.pathExtension.lowercased() == "3105" {
            do {
                ...
                if let contentKey = try PatchKeyStore.load(for: summary),
                   let cachedPackage = try? PatchPackageCodec.decode(data, contentKey: contentKey) {
                    decoded = cachedPackage
                } else if summary.isPasswordProtected {
                    // Only bundled resources use the internal key; imported
                    // packages remain locked for the user.
                    let filename = url.deletingPathExtension().lastPathComponent
                    let isBundledResource = filename.hasPrefix("xTop1 External File (")
                        || [
                            "ATHBODY", "ATHDRAG", "ATHMAGIC",
                            "ATHBODYM", "ATHDRAGM", "ATHMAGICM",
                            "ATHBODYFFTH", "ATHHEADFFTH", "ATHANTENA-DRAG",
                            "ATHBODYFFM", "ATHDRAGFFM", "ATHANTENA-HEADFFM"
                        ].contains(filename)
                    guard isBundledResource else {
                        decoded = nil
                        continue
                    }
                    var bundled: DecodedPatchPackage?
                    for password in PatchPackageCodec.bundledResourcePasswords {
                        if let candidate = try? PatchPackageCodec.decode(
                            data,
                            password: password
                        ) {
                            bundled = candidate
                ...
                byFilename[url.lastPathComponent.lowercased()] = item
            } catch {
